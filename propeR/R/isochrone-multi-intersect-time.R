...
isochroneMultiIntersectTime <- function(output.dir,
  ...
  for (num in 1:length(time_series)){ # Start loop to calculate journey details
    for (i in 1:nrow(originPoints)){ # Start loop to calculate journey details
      ...
      date_time_legend <- format(time_series[num], "%B %d %Y %H:%M") # Creates a legend value for date in day, month, year and time in 24 clock format
  ...
  m <- magick::image_read(paste0(output.dir,"/isochrone_time_intersect-",stamp,".gif")) %>%
