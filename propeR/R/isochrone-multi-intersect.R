...
isochroneMultiIntersect <- function(output.dir,
  ...
  palIsochrone=colorFactor(palColorMarker, NULL, n=length(originPoints)) # Creating colour palette
  ...
  for (i in 1:nrow(originPoints)){ #Changes transport modes to OTP transport modes
    ...
    isochrone <- propeR::otpIsochrone( 
      ...
      date = from_origin$date, # Takes the date as specified above
      time = from_origin$time, # Takes the time as specified above
  ...
  message("Finding the intersect between ",nrow(originPoints)," isochrones, please wait...")
  time.taken <- list(0)
  ...
  for (n in 1:length(isochrone_multi$status)){
    ...
    if (n == 1){
      isochrone_polygons <- rgdal::readOGR(isochrone_multi$response[1], "OGRGeoJSON", verbose = FALSE) # Reads first response and greates SpatialPolygonsDataFrame
      ...
      s_poly_all <- s_poly
    } else { # Cleans and appends all other SpatialPolygonsDataFrames together
      isochrone_polygons_tmp <- rgdal::readOGR(isochrone_multi$response[n], "OGRGeoJSON", verbose = FALSE)
    ...
    if (n < length(isochrone_multi$status)){
      message(n," out of ",length(isochrone_multi$status),
  ...
  popup_originPoints <- # generates a popup for the poly_lines_lines feature
    paste0("<strong>Name: </strong>",
           originPoints$name,
           "<br><strong>Mode: </strong>",
           originPoints$mode,
           "<br><strong>Duration: </strong>",
           round(originPoints$max_duration, digits = 2), " mins",
  ...
  m <- setView(m, lat=mean(originPoints$lat), # Focuses on the origin
               lng=mean(originPoints$lon), # Focuses on the origin
               zoom=mapZoom)
  m <- addPolygons(m, data = s_poly_intersect, # Adds polygons from journey
                   stroke = TRUE,
                   weight = 5,
  ...
  m <- addCircleMarkers(m, data = originPoints, # Adds circles for each stage of the journey
                        lat = ~lat,
                        lng = ~lon,
                        radius = 8,
  ...
  m <- addLegend(m, pal = palIsochrone, # Adds a legend for the trip
                 values = paste(originPoints$name," by ",originPoints$mode," in ", originPoints$max_duration," mins", sep=""),
                 opacity = 0.8)
  ...
  mapview::mapshot(m, file = paste0(output.dir, "/isochrone_multi_intersect-",stamp,".png"))
  htmlwidgets::saveWidget(m, file = paste0(output.dir, "/isochrone_multi_merge-",stamp,".html")) # Saves as an interactive HTML webpage
  unlink(paste0(output.dir,"/isochrone_multi_merge-",stamp,"_files"), recursive = TRUE) # Deletes tmp_folder
  
  s_poly_intersect <- as(s_poly_intersect, "SpatialPolygonsDataFrame")
  rgdal::writeOGR(s_poly_intersect, dsn = paste0(output.dir, "/isochrone_multi_intersect-",stamp,".geoJSON"), layer="s_poly_intersect",driver="GeoJSON")
}
