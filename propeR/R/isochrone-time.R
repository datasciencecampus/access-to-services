...
isochroneTime <- function(output.dir,
  ...
  destination_points_num_of_cols <- ncol(destinationPoints) # Need for later to distinguish original column length
  destination_points_output <- destinationPoints # Creates a copy so doesn't overwrite original data, so script can be rerun without loading data again
  destination_points_output[as.character(time_series)] <- NA # Creates column for each time_series which we will get journey time for later
  
  message("Creating ",length(time_series)," isochrones, please wait...")
  time.taken <- list(0)
  ...
  for (i in 1:length(time_series)){ # Start loop to calculate journey details
    ...
    destination_points_spdf <- destination_points_output # Create a spatialdataframe
    ...
    isochrone_polygons_split <- sp::split(isochrone_polygons,isochrone_polygons@data$time) # Split into cutoff times
    ...
    for (n in 1:nrow(destination_points_spdf)){ # Finds the smallest duration from time_df dataframe
      if (is.na(time_df[n,length(isochroneCutOffs)])){
        ...
        time_df[n,length(isochroneCutOffs)+1] = min(time_df[n,1:length(isochroneCutOffs)],na.rm=TRUE)
    ...
    m <- setView(m, lat=from_origin$lat, # Focuses on the origin
                 ...
                 zoom=mapZoom) 
    m <- addPolygons(m, data = isochrone_polygons,
                     stroke = TRUE,
    ...
    m <- addCircleMarkers(m, data = destinationPoints,
                          lat = ~lat,
                          lng = ~lon,
                          fillColor = "white",
                          stroke = TRUE,
                          ...
                          fillOpacity = 1,
                          radius = 10)
    m <- addCircleMarkers(m, data = destination_points_non_na,
                          lat = ~lat,
                          lng = ~lon,
                          fillColor = "black",
                          stroke = TRUE,
    ...
    m <- addLegend(m, pal = pal_time_date, # Adds a legend for the time
  ...
  m <- magick::image_read(paste0(output.dir,"/isochrone_time-",stamp,".gif")) %>%
    magick::image_scale("600") # Loads GIF into R
  
  invisible(print(m)) # plots map to Viewer
  
  write.csv(destination_points_output, file = paste0(output.dir,"/output_isochrone_inc-",stamp,".csv"),row.names=FALSE) # Saves trip details as a CSV
