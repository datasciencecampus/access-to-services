...
isochroneMulti <-function(output.dir,
  ...
  destination_points_spdf <- destinationPoints # Create a spatialdataframe
  sp::coordinates(destination_points_spdf) <- ~ lon + lat # Take lat and lon for coordinates
  ...
  for (i in start_num:end_num){ # Start loop to calculate journey details
    ...
    isochrone <- propeR::otpIsochrone( 
      otpcon,
      batch = TRUE, # If true, goal direction is turned off and a full path tree is built (specify only once)
    ...
    if (i == start_num){ # Combines outputs
      t <- try(isochrone_polygons <- rgdal::readOGR(isochrone$response, "OGRGeoJSON", verbose = FALSE)) # Converts the polygons to be handled in leaflet
      if ("try-error" %in% class(t)){next
      }else{
        isochrone_polygons <- rgdal::readOGR(isochrone$response, "OGRGeoJSON", verbose = FALSE)
      }
      isochrone_polygons <- rgdal::readOGR(isochrone$response, "OGRGeoJSON", verbose = FALSE) # Converts the polygons to be handled in leaflet
      isochrone_polygons@data$name <- from_origin$name # adds name to json
      sp::proj4string(destination_points_spdf) <- sp::proj4string(isochrone_polygons) # Take projection
      time_df_tmp <- data.frame(matrix(, ncol = nrow(destinationPoints), nrow = 1)) # Create time dataframe
      isochrone_polygons_split <- sp::split(isochrone_polygons,isochrone_polygons@data$time) # Split into names
      ...
      for (n in 1:ncol(time_df_tmp)){
        time_df_tmp[length(isochroneCutOffs)+1,n] <- min(time_df_tmp[1:length(isochroneCutOffs),n],na.rm = TRUE)
      }
      options(warn = 0)
      rownames(time_df_tmp)[length(isochroneCutOffs)+1] <- originPoints$name[i]
      time_df <- time_df_tmp[length(isochroneCutOffs)+1,]
    } else {
      t <- try(isochrone_polygons_tmp <- rgdal::readOGR(isochrone$response, "OGRGeoJSON", verbose = FALSE)) # Converts the polygons to be handled in leaflet
      if ("try-error" %in% class(t)){next
      } else {
        isochrone_polygons_tmp <- rgdal::readOGR(isochrone$response, "OGRGeoJSON", verbose = FALSE)
      }
      ...
      time_df_tmp <- data.frame(matrix(, ncol = nrow(destinationPoints), nrow = 1)) # Create time dataframe
      isochrone_polygons_split <- sp::split(isochrone_polygons_tmp,isochrone_polygons_tmp@data$time) # Split into names
      for (n in 1:length(isochroneCutOffs)){
        time_df_tmp2 <- sp::over(destination_points_spdf, isochrone_polygons_split[[n]]) # Finds the polygon the destination point falls within
        time_df_tmp2$index <- as.numeric(row.names(time_df_tmp2))
        time_df_tmp2 <- time_df_tmp2[order(time_df_tmp2$index), ]
  ...
  m <- setView(m, lat=from_origin$lat, # Focuses on the origin
               lng=from_origin$lon, # Focuses on the origin
               zoom=mapZoom) 
  m <- addPolygons(m, data = isochrone_polygons, # Adds polygons from journey
                   stroke = TRUE,
  ...
  m <- addCircleMarkers(m, data = destinationPoints, # Adds circles for each destination
                        lat = ~lat,
                        lng = ~lon,
                        fillColor = "white",
                        stroke = TRUE,
                        ...
                        fillOpacity = 1,
                        radius = 10)
  m <- addLegend(m, pal = palIsochrone, # Adds a legend for the trip
                 values = isochroneCutOffs,
                 opacity = 0.5,
                 title = "Duration (minutes)")
  m <- addAwesomeMarkers(m, data = originPoints, # Adds the origin as a marker
                         lat = ~lat,
                         lng = ~lon,
                         popup = ~name,
  ...
  unlink(paste0(output.dir,"/isochrone_multi-",stamp,"_files"), recursive = TRUE) # Deletes temporary folder created by mapshot
  
  write.csv(time_df, file = paste0(output.dir,"/isochrone_multi_inc-",stamp,".csv"),row.names=TRUE) # Saves trip details as a CSV
