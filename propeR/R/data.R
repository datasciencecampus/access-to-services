...
importLocationData <- function(src) {
  ...
  names(data_points) <- tolower(names(data_points)) # Convert column names to lower case
  if("lat" %in% colnames(data_points)) # Checks data to see if lat and lon columns are present
  {
    ...
    message("Will try and find a postcode column in the data. If one is found, it may be able to be converted to latitude and longitude.\n"); # Checks to see if postcode has been given, if so we can convert to lat, lon
    if("postcode" %in% colnames(data_points)){
      message("Postcodes were found, will convert to latitude and longitude now. Warning: If a postcode doesn't exist 
      ...
      for (i in 1:nrow(data_points)){
        pc_content <- propeR::postcodeToDecimalDegrees(data_points$postcode[i])
        if (pc_content$status == '404') {
          warning("Warning: Postcode", data_points$name[i], "cannot be convert to a latitude and longitude. 
      ...
      stop("A postcode column cannot be found, please review the data and either provide a latitude 'lat' and longitude 
           'lon' column (best), or postcode, for each location.\n");
    }
    }    
  data_points <- data_points[order(data_points$name),] # Sort by name
  data_points <- as.data.frame(data_points) # Converts to dataframes and create lat, lon field needed by otp
  data_points$lat_lon <- with(data_points, paste0(lat,",", lon)) # Adds a lat_lon column as needed by otp
  data_points
}
