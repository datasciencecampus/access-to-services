...
pointToPointTime <- function(output.dir,
  ...
  for (i in 1:length(time_series)) {
    ...
    if (i == 1) {
      ...
      point_to_point_output_table <-
        rbind(point_to_point_output_table,
  ...
  for (i in 1:nrow(point_to_point_output_table)) {
    if (i == 1) {
      ...
      if (point_to_point_output_table$start[i] > point_to_point_output_table$start[i -
                                                                                   1]) {
        point_to_point_output_table$date[i] = point_to_point_output_table$date[i -
                                                                                 1]
      } else {
        point_to_point_output_table$date[i] = format(as.Date(
          strptime(point_to_point_output_table$date[i - 1], "%m/%d/%Y")
        ) + 1, "%m/%d/%Y")
      }
    }
  ...
  point_to_point_output_table$date_time <-
    paste0(point_to_point_output_table$date,
           " ",
           point_to_point_output_table$start)
