...
choropleth <- function(output.dir,
  ...
  pal_choropleth_cat = leaflet::colorFactor(palColorCat, na.color = "#ffffff", domain =
                                              NULL) # Creating colour palette
  ...
  for (i in 1:nrow(originPoints)) {
    # Loops for the number of origin points
    ...
    if (choropleth$errorId == "OK") {
      ...
      originPoints[i, "waitingtime"] <-
        choropleth$itineraries$waitingTime
      originPoints[i, "transfers"] <-
        choropleth$itineraries$transfers
      
      if (originPoints[i, "duration"] > (durationCutoff)) {
        # Apply duration cutoff
        originPoints[i, "duration_cat"] <-
          sprintf('Over %s minutes', durationCutoff)
      } else {
        originPoints[i, "duration_cat"] <-
          sprintf('Under %s minutes', durationCutoff)
      }
      
      if (originPoints[i, "waitingtime"] > (waitingCutoff)) {
        # Apply waiting time cutoff
        originPoints[i, "waitingtime_cat"] <-
          sprintf('Over %s minutes', waitingCutoff)
      } else {
        originPoints[i, "waitingtime_cat"] <-
          sprintf('Under %s minutes', waitingCutoff)
      }
      
      if (originPoints[i, "transfers"] >= (transferCutoff)) {
        # Apply transfers cutoff
        originPoints[i, "transfers_cat"] <-
          sprintf('Over %s transfer(s)', transferCutoff)
      } else {
        originPoints[i, "transfers_cat"] <-
          sprintf('Under %s transfer(s)', transferCutoff)
      }
    ...
    if (i < nrow(originPoints)) {
      message(
        ...
        round(do.call(sum, time.taken), digits = 2),
        " seconds. Estimated time left is approx. ",
        round((
          do.call(mean, time.taken) * nrow(originPoints)
        ) - do.call(sum, time.taken), digits = 2),
        " seconds."
      )
    } else {
      message(
  ...
  choropleth_map <-
    sp::merge(originPolygons, originPoints[, c(
      'name',
      'duration',
      'waitingtime',
      'transfers',
      ...
      'waitingtime_cat',
      'transfers_cat'
    )], by = c("name")) # Merges the information from the input polygon and the function called
  choropleth_table <-
    originPoints[, c(
      'name',
      'duration',
      ...
      'waitingtime_cat',
      'transfers_cat'
    )] # Creates a table of information based on the function
  ...
  popup_waitingtime <-
    # Creates a popup for waiting time for leaflet
    paste0(
      "<strong>Name: </strong>",
      choropleth_map$name,
  ...
  m <- setView(m,
               lat = to_destination$lat,
               # Focuses on destination
               lng = to_destination$lon,
               # Focuses on destination
               zoom = mapZoom)
  m <-
    addPolygons(
  ...
  m_cat <-
    addProviderTiles(m_cat, providers$OpenStreetMap.BlackAndWhite)
  m_cat <-
    setView(m_cat,
            lat = to_destination$lat,
            # Focuses on destination
  ...
  n_cat <-
    addProviderTiles(n_cat, providers$OpenStreetMap.BlackAndWhite)
  n_cat <-
    setView(n_cat,
            lat = to_destination$lat,
            # Focuses on destination
  ...
  o_cat <-
    addProviderTiles(o_cat, providers$OpenStreetMap.BlackAndWhite)
  o_cat <-
    setView(o_cat,
            lat = to_destination$lat,
            # Focuses on destination
  ...
  htmlwidgets::saveWidget(m,
                          file = paste0(output.dir, "/choropleth_duration-", stamp, ".html")) # Saves as an interactive HTML webpage
  unlink(paste0(output.dir, "/choropleth_duration-", stamp, "_files"),
         recursive = TRUE) # Deletes temporary folder that mapshot creates
  invisible(print(m)) # plots map to Viewer
  
  mapview::mapshot(m_cat,
                   file = paste0(output.dir, "/choropleth_duration_cat-", stamp, ".png"))
  htmlwidgets::saveWidget(m_cat,
                          file = paste0(output.dir, "/choropleth_duration_cat-", stamp, ".html")) # Saves as an interactive HTML webpage
  unlink(paste0(output.dir, "/choropleth_duration_cat-", stamp, "_files"),
         recursive = TRUE) # Deletes temporary folder that mapshot creates
  invisible(print(m_cat)) # plots map to Viewer
  
  mapview::mapshot(n,
                   file = paste0(output.dir, "/choropleth_waitingtime-", stamp, ".png"))
  htmlwidgets::saveWidget(n,
                          file = paste0(output.dir, "/choropleth_waitingtime-", stamp, ".html")) # Saves as an interactive HTML webpage
  unlink(paste0(output.dir, "/choropleth_waitingtime-", stamp, "_files"),
         recursive = TRUE) # Deletes temporary folder that mapshot creates
  invisible(print(n)) # plots map to Viewer
  
  mapview::mapshot(n_cat,
                   file = paste0(output.dir, "/choropleth_waitingtime_cat-", stamp, ".png"))
  htmlwidgets::saveWidget(n_cat,
                          file = paste0(output.dir, "/choropleth_waitingtime_cat-", stamp, ".html")) # Saves as an interactive HTML webpage
  unlink(paste0(output.dir, "/choropleth_waitingtime_cat-", stamp, "_files"),
         recursive = TRUE) # Deletes temporary folder that mapshot creates
  invisible(print(n_cat)) # plots map to Viewer
  
  mapview::mapshot(o,
                   file = paste0(output.dir, "/choropleth_transfers-", stamp, ".png"))
  htmlwidgets::saveWidget(o,
                          file = paste0(output.dir, "/choropleth_transfers-", stamp, ".html")) # Saves as an interactive HTML webpage
  unlink(paste0(output.dir, "/choropleth_transfers-", stamp, "_files"),
         recursive = TRUE) # Deletes temporary folder that mapshot creates
  invisible(print(o)) # plots map to Viewer
  
  mapview::mapshot(o_cat,
                   file = paste0(output.dir, "/choropleth_transfers_cat-", stamp, ".png"))
  htmlwidgets::saveWidget(o_cat,
                          file = paste0(output.dir, "/choropleth_transfers_cat-", stamp, ".html")) # Saves as an interactive HTML webpage
  unlink(paste0(output.dir, "/choropleth_transfers_cat-", stamp, "_files"),
         recursive = TRUE) # Deletes temporary folder that mapshot creates
