...
isochroneMultiIntersectSensitivity <- function(output.dir,
  ...
  if (is.null(originPoints$max_time)) {
    originPoints$max_time <- isochroneCutOffs
  }
  ...
  for (num in 1:length(time_series)) {
    ...
    for (i in 1:nrow(originPoints)) {
      ...
      time <-
        as.POSIXct(from_origin$time, format = "%I:%M %p") + time_series[num]
      date <- from_origin$date
      modes <- from_origin$mode
      max_duration_isochrone <-
        from_origin$max_time # Set maximum journey time in minutes
    ...
    for (n in 1:length(isochrone_multi$status)) {
      if (n == 1) {
        ...
        s_poly <-
          SpatialPolygonsDataFrame(isochrone_polygons, poly_df) # Merges back to SpatialPolygonsDataFrame
  ...
  list.files(
    ...
    magick::image_write(paste0(
      output.dir,
      "/isochrone_intersect_sensitivity-",
      stamp,
      ".gif"
    )) # write to current dir
  
  m <-
    magick::image_read(paste0(
      output.dir,
      "/isochrone_intersect_sensitivity-",
      stamp,
      ".gif"
