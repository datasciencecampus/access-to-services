...
##' 
##' Calculates an isochrone map for multiple origins and finds
##' the intersection including a sensitivity of 30 minutes either side of
##' specified journey in 5 minute increments.
##'
...
##' @param timeSensitivity in minutes, defaults to 30
##' @param timeSensitivityStep in minutes, defaults to 5
...
isochroneMultiIntersectSensitivity <- function(output.dir,
  ...
  num_origin_points <- nrow(originPoints)
  
  # Tidying variables ----------
  if (is.null(originPoints$mode)){ originPoints$mode <- modes }
  if (is.null(originPoints$max_time)){ originPoints$max_time <- isochroneCutOffs }
  ...
  message("Creating ",nrow(originPoints)*length(time_series)," isochrones, please wait...\n")
  
  isoNum <- 0
  
  # SCRIPT
  for (num in 1:length(time_series)){ # Start loop to calculate journey details
    for (i in 1:nrow(originPoints)){ # Start loop to calculate journey details
      ...
      max_duration_isochrone <- from_origin$max_time # Set maximum journey time in minutes
      ...
      message(isoNum," out of ",nrow(originPoints)*length(time_series),
              " isochrones complete.")
  ...
  list.files(path = paste0(output.dir,"/tmp_folder"), pattern = "*.png", full.names = T) %>%  # Creates gif of results
    ...
    magick::image_write(paste0(output.dir,"/isochrone_intersect_sensitivity-",stamp,".gif")) # write to current dir
  
  m <- magick::image_read(paste0(output.dir,"/isochrone_intersect_sensitivity-",stamp,".gif")) %>%
